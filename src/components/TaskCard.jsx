...
function TaskCard({ task, columnId, isOverlay }) {
  ...
  return (
    <div
      ...
      {...(!isEditing && listeners)}
      className={`group mt-2 relative flex flex-col justify-between cursor-grab rounded-lg bg-indigo-200 p-4 shadow-sm ring-1 ring-indigo-700 transition-all hover:ring-indigo-600 hover:shadow-md 
                ${
        isOverlay
    ...
    >
      ...
      {isEditing ? (
        ...
        <>
          ...
          <div>
                       
            <h3 className="pr-14 font-semibold text-neutral-800 leading-tight">
                            {task.title}           
            </h3>
                       
          ...
          {!isOverlay && (
            <div className="absolute top-3 right-3 flex gap-2 opacity-0 transition-opacity group-hover:opacity-100">
              ...
              <button
                ...
                className="rounded p-1.5 text-neutral-800 hover:bg-indigo-700 hover:text-gray-400 transition-colors cursor-pointer"
                title="Edit Task"
              >
                                <SquarePen size={20} />             
              </button>
              ...
              <button
                ...
                onPointerDown={(e) => e.stopPropagation()}
                className="rounded p-1.5 text-neutral-800 hover:bg-indigo-700 hover:text-red-400 transition-colors cursor-pointer"
                title="Delete Task"
              >
                                <Trash size={20} />             
              </button>
